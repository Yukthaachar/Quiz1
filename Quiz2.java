...
public class Quiz2 {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for(int i=0;i<size;i++){
            System.out.println(quesnames.get(choice-1)+" "+mylist1.get(i).get()[0]);
            String input=sc4.nextLine();
            if(mylist1.get(i).get()[1].equalsIgnoreCase(input)){
                total=total+marks;
            }
            else{
                total=total+0;
                wrques.add(mylist1.get(i).get()[0]);
                wrans.add(mylist1.get(i).get()[1]);
            }
        }
